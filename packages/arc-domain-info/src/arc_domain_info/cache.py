...
def cache_root() -> Path:
    if value := os.environ.get("ARC_DOMAIN_INFO_CACHE"):
        return Path(value).expanduser()
    if value := os.environ.get("XDG_CACHE_HOME"):
        return Path(value).expanduser() / "arc" / "domain-info"
    if project_root := _project_root():
        return project_root / "cache" / "domain-info"
    return Path.home() / ".cache" / "arc" / "domain-info"
...
def _project_root() -> Path | None:
    for parent in Path(__file__).resolve().parents:
        if (parent / "packages" / "arc-domain-info").is_dir() and (parent / "cache").is_dir():
            return parent
    return None
