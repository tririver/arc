...
def test_cli_does_not_expose_workflow_consensus_command():
    parser = cli._build_parser()

    try:
        parser.parse_args(["proposers-reviewer-consensus", "--config", "config.json"])
    except SystemExit as exc:
        assert exc.code == 2
    else:
        raise AssertionError("arc-llm must not expose workflow consensus command")
