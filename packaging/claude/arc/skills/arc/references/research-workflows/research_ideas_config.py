...
GLOBAL_REVIEW_DEFAULT_TEMPLATE = (
    "Review all proposed ideas on one common 0-100 scale against the full user intent. "
    "Use only the supplied proposal records, novelty-check notes, and included context. Do not use tools. "
    "Calibrate each mark as a percentage of that dimension's maximum before converting it to points. "
    "Ordinary competent ideas should be around 60% of each applicable dimension, not 80-90%. "
    "Do not be afraid to assign less than 60% when an idea is incremental, vague, weakly evidenced, "
    "or only loosely feasible. Above 90% in any dimension is reserved for genuinely outstanding "
    "performance in that dimension; for example, evidence_of_novelty above 90% requires extremely "
    "strong evidence for a real research gap, and first_calculation_clarity above 90% requires a nearly "
    "execution-ready calculation with clear setup, variables, approximations, deliverables, and validation checks. "
    "In each review item's marks, give evidence_of_novelty on a 0-30 scale: 30 means you are completely "
    "confident from the supplied evidence that the idea has not been done before, and 0 means you are "
    "completely confident it has already been done. This is evidence for the binary done/not-done question, "
    "not a score for how exotic the idea feels. Give user_intent_relevance on a 0-30 scale against the full "
    "user intent, including both the requested calculation and the intended research field/domain when "
    "specified. Give scientific_value and feasibility on a 0-15 scale, and first_calculation_clarity on a "
    "0-10 scale. Set total_score to the closest integer after summing evidence_of_novelty, "
    "user_intent_relevance, scientific_value, feasibility, and first_calculation_clarity, so total_score is "
    "0-100. Rank all ideas by this total_score and use the same scale across variants. Return exactly one "
