...
**Goal:** Rebuild `arc-paper-query` as a cache-first Python package with CLI, LLM summary generation, resumable batch processing, MCP tools, and Codex/Claude plugin wrappers.

**Architecture:** `arc-paper-query` owns all deterministic paper data, caching, parsing, LLM summary contracts, provider selection, and batch execution. MCP, skills, and plugins are thin adapters over the CLI/package; plugins inject `ARC_AGENT_HOST` and `ARC_LLM_PROVIDER`, with parent-process detection only as fallback. The first implementation targets ar5iv full text and INSPIRE metadata/references/citers only.
...
- Use `0_ref/skills/arc/utils/paper-query/` as the reference implementation for provider endpoints, old cache behavior, parser heuristics, and CLI affordances. Read it before implementing each related component, but do not preserve compatibility and do not copy the old OpenAlex/Semantic Scholar/network-building design.
- No backwards compatibility with old `paper-query`.
...
### `packages/arc-paper-query`

- `pyproject.toml`: package metadata, dependencies, console script.
- `src/arc_paper_query/ids.py`: paper ID parsing and normalization.
- `src/arc_paper_query/results.py`: stable JSON result envelope.
- `src/arc_paper_query/cache.py`: cache paths, TTL rules, JSON/HTML read-write helpers.
- `src/arc_paper_query/host.py`: env-first host/provider detection, parent-process fallback on Linux/macOS/Windows.
- `src/arc_paper_query/providers/base.py`: provider protocols and errors.
- `src/arc_paper_query/providers/ar5iv.py`: ar5iv HTML download/cache.
- `src/arc_paper_query/providers/inspire.py`: INSPIRE metadata/references/citers download/cache.
- `src/arc_paper_query/parse/ar5iv_html.py`: HTML-to-text, TOC, section extraction.
- `src/arc_paper_query/parse/equations.py`: equation context extraction.
- `src/arc_paper_query/service.py`: public Python API for single/list paper queries.
- `src/arc_paper_query/cli.py`: CLI parser and JSON output.
- `src/arc_paper_query/summary/schema.py`: summary JSON schema loader and validator.
- `src/arc_paper_query/summary/input_pack.py`: deterministic input pack builder for LLM summary.
- `src/arc_paper_query/summary/store.py`: summary cache read/write and source-hash validation.
- `src/arc_paper_query/summary/providers/*.py`: LLM provider abstraction and implementations.
- `src/arc_paper_query/batch/db.py`: SQLite queue and status tracking.
- `src/arc_paper_query/batch/runner.py`: resumable concurrent batch runner.
...
- `examples/paper-query/papers.txt`: sample IDs for batch testing.

### Adapters

- `packages/arc-mcp/src/arc_mcp/server.py`: MCP tool server over `arc_paper_query.service`.
- `skills/arc/SKILL.md`: short agent workflow for ARC research and paper-query use.
- `skills/arc/references/paper-query.md`: detailed `needs_llm`, batch, and troubleshooting workflow.
...
## Milestone 1: Deterministic `arc-paper-query`

### Task 1: Package Skeleton And Test Harness

**Files:**
- Create: `packages/arc-paper-query/pyproject.toml`
- Create: `packages/arc-paper-query/src/arc_paper_query/__init__.py`
- Create: `packages/arc-paper-query/tests/test_import.py`
...
[project.scripts]
arc-paper-query = "arc_paper_query.cli:main"
```
...
def test_package_imports():
    import arc_paper_query

    assert arc_paper_query.__version__ == "0.1.0"
```
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/ids.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/results.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/cache.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/host.py`
- Create: `packages/arc-paper-query/tests/test_ids.py`
- Create: `packages/arc-paper-query/tests/test_results.py`
- Create: `packages/arc-paper-query/tests/test_host.py`
...
```python
from arc_paper_query.ids import normalize_paper_id, arxiv_path_id
...
```python
from arc_paper_query.results import ok, err
...
```text
ARC_PAPER_QUERY_CACHE if set
else XDG_CACHE_HOME/arc/paper-query
else ~/.cache/arc/paper-query
```
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/providers/__init__.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/providers/base.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/providers/ar5iv.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/providers/inspire.py`
- Create: `packages/arc-paper-query/tests/test_providers_ar5iv.py`
- Create: `packages/arc-paper-query/tests/test_providers_inspire.py`
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/parse/__init__.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/parse/ar5iv_html.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/parse/equations.py`
- Create: `packages/arc-paper-query/tests/fixtures/ar5iv_sample.html`
- Create: `packages/arc-paper-query/tests/test_parse_ar5iv.py`
- Create: `packages/arc-paper-query/tests/test_equations.py`
...
```python
from arc_paper_query.parse.ar5iv_html import parse_html, get_section
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/service.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/cli.py`
- Create: `packages/arc-paper-query/tests/test_service.py`
- Create: `packages/arc-paper-query/tests/test_cli.py`
...
arc-paper-query get-references arXiv:0911.3380 arXiv:hep-th/0601001 --json
...
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/__init__.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/schema.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/input_pack.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/store.py`
- Create: `packages/arc-paper-query/tests/test_summary_schema.py`
- Create: `packages/arc-paper-query/tests/test_summary_store.py`
...
```json
{
  ...
  "next": {
    "store_command": "arc-paper-query store-llm-summary arXiv:0911.3380 --summary-json -"
  }
}
```
...
```bash
arc-paper-query store-llm-summary arXiv:0911.3380 --summary-json summary.json --json
```
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/__init__.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/base.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/manual.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/codex_cli.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/claude_cli.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/summary/providers/select.py`
- Create: `packages/arc-paper-query/tests/test_provider_selection.py`
- Create: `packages/arc-paper-query/tests/test_host_cli_providers.py`
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/batch/__init__.py`
- Create: `packages/arc-paper-query/src/arc_paper_query/batch/db.py`
- Create: `packages/arc-paper-query/tests/test_batch_db.py`
- Create: `examples/paper-query/papers.txt`
...
**Files:**
- Create: `packages/arc-paper-query/src/arc_paper_query/batch/runner.py`
- Create: `packages/arc-paper-query/tests/test_batch_runner.py`
...
```bash
arc-paper-query summary-batch run qft-ideas --provider auto --concurrency 2 --resume --json
```
...
```bash
arc-paper-query summary-batch run qft-ideas --provider auto --max-items 10 --concurrency 1 --json
```
...
```bash
arc-paper-query summary-batch export qft-ideas --format jsonl --output summaries.jsonl
...
- Create: `skills/arc/references/paper-query.md`
...
```text
Use `arc-paper-query` or MCP tools for paper data.
...
```bash
ARC_AGENT_HOST=codex ARC_LLM_PROVIDER=codex-cli arc-paper-query ...
```

Claude plugin skill should prefer:

```bash
ARC_AGENT_HOST=claude-code ARC_LLM_PROVIDER=claude-cli arc-paper-query ...
```
...
```text
pip install -e packages/arc-paper-query
