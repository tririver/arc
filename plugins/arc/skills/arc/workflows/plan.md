...
Add `[human-resolved]` beside the content, and only color `human-resolved` blue. Do not color the surrounding prose. Do not color the surrounding equations. If color is stripped or unavailable, the marker remains authoritative. For PDF-oriented Markdown, write raw LaTeX directly in prose, for example: [\textcolor{blue}{human-resolved}]. Do not use Markdown code spans.

Step 6: For note-check content classified as `confirmed_source_error`, mark the source issue with the literal marker `[confirmed source issue]`, and only color that marker red. Do not color the surrounding prose. Do not color the surrounding equations. For `likely_source_error` or `ambiguous_convention`, use red only for the `calculate.md` auto-mode `[agent-resolved decision]` marker; otherwise ask a human expert question before acceptance. For PDF-oriented Markdown, write raw LaTeX directly in prose, for example: \textcolor{red}{[confirmed source issue]}. Do not use Markdown code spans.
